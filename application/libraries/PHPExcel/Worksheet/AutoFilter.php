            ...
            switch ($filterColumn->getFilterType()) {
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER:
                    ...
                    foreach ($rules as $rule) {
                        ...
                        if ($toptenRuleType === PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_TOPTEN_PERCENT) {
                            // Set Percentage based TopTen rule
                            $rank = intval(($ruleValue / 100) * $dataRowCount);
                            $ruleValues = array(
                                array('operator' => PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_PERCENT, 'value' => $rank)
                            );
                        } else {
                            // Set Number based TopTen rule
                            $rank = intval($ruleValue);
                            $ruleValues = array(
                                array('operator' => PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_TOPTEN_ITEMS, 'value' => $rank)
                            );
                        }
                        $columnFilterTests[$columnID] = array(
                            'method' => 'filterTestInTop10DataSet',
                            'arguments' => array('filterValues' => $ruleValues, 'blanks' => true)
                        );
                    }
                    break;
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER:
                    ...
                    break;
                default:
                    throw new PHPExcel_Exception('Invalid filter type.');
            }
        }
