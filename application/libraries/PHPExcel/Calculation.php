...
class PHPExcel_Calculation
{
    ...
    public const CALCULATION_REGEXP_FUNCTION    = '@?([A-Z][A-Z0-9\.]*)[\s]*\(';
    //    Cell reference (cell or range of cells, with or without a sheet reference)
    public const CALCULATION_REGEXP_CELLREF    = CALCULATION_REGEXP_CELLREF;
    //    Named Range of cells
    public const CALCULATION_REGEXP_NAMEDRANGE    = CALCULATION_REGEXP_NAMEDRANGE;
    //    Error
    ...
    public function _calculateFormulaValue($formula, $cellID = null, PHPExcel_Cell $pCell = null)
    {
        ...
        // Further implementation of formula calculation goes here
    ...
    private function showTypeDetails($value)
    {
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            if ($value === null) {
                ...
                } elseif ($value[0] == '#') {  // Mengganti $value{0} menjadi $value[0]
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
{
    ...
    // Store the parent worksheet for potential re-attachment
    $pCellParent = ($pCell !== null) ? $pCell->getWorksheet() : null;

    // Regular expression for matching different components of the formula
    ...
    while ($index < strlen($formula)) {
        ...
        // Determine if it's an operand or function
        $isOperandOrFunction = preg_match($regexpMatchString, substr($formula, $index), $match);

        // Handle different cases based on $opCharacter and $isOperandOrFunction
        if ($opCharacter == '-' && !$expectingOperator) {
            ...
            // Handle operands, variables, etc.
            // ...

        } elseif ($opCharacter == '$') {
            // Handle absolute row or column range
            ++$index;

        } elseif ($opCharacter == ')') {
            // Miscellaneous error checking for unexpected ')'
            if ($expectingOperand) {
        ...
        ++$index; // Move to the next character in the formula
    }

    // Handle end of formula string
    if ($expectingOperator) {
        return $this->raiseFormulaError("Formula Error: Unexpected end of formula");
    }

    // Finalize processing and return output
    // ...
