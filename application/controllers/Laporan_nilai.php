		...
		function nilai_semester(){
	       ...
	       $siswa = $this->db->query($sqlSiswa)->row_array();
			...
			foreach ($mapel as $m) {
				...
				$rataRataNilai = $this->rata_rata_nilai($m->id_jadwal);
				if (is_null($rataRataNilai)) {
					$rataRataNilai = 0; // atau nilai default lainnya
				}
				$pdf->Cell(20,5,  ceil($rataRataNilai),1,0,'L');
